-- Primeiro, vamos verificar o tipo da coluna 'type' na tabela jobs
SELECT column_name, data_type, udt_name 
FROM information_schema.columns 
WHERE table_name = 'jobs' AND column_name = 'type';

-- Corrigir a função create_job_from_request para lidar com o tipo correto
...
BEGIN
    ...
    -- Criar nova vaga na tabela jobs com conversão de tipo adequada
    INSERT INTO public.jobs (
        ...
        request_data.city, 
        request_data.state,
        request_data.type::contract_type, -- Conversão explícita para o tipo enum
...
EXCEPTION
    WHEN OTHERS THEN
        -- Se houver erro de conversão, tentar com valores padrão
        INSERT INTO public.jobs (
            title,
            department, 
            city,
            ...
            request_data.city, 
            request_data.state,
            'CLT'::contract_type, -- Valor padrão se conversão falhar
            request_data.description,
            request_data.requirements,
            request_data.benefits,
            request_data.workload,
