-- Corrigir validação de contract_type na função create_job_from_request
-- O problema é que valores inválidos como "153 - Sul" (que é company_contract) estão sendo enviados para o campo type
...
BEGIN
    ...
    -- Validar e corrigir o tipo de contrato
    -- Valores válidos: 'CLT', 'Estágio', 'Aprendiz', 'Terceirizado', 'Temporário', 'PJ', 'Freelancer'
    BEGIN
        -- Tentar fazer cast direto
        validated_type := request_data.type::contract_type;
    EXCEPTION WHEN OTHERS THEN
        -- Se falhar, usar 'CLT' como padrão
        validated_type := 'CLT';
        RAISE WARNING 'Tipo de contrato inválido "%" na solicitação % - usando CLT como padrão', request_data.type, request_id;
    END;
    ...
    INSERT INTO public.jobs (
        ...
        request_data.city, 
        request_data.state,
        validated_type, -- Usar tipo validado
...
COMMENT ON FUNCTION public.create_job_from_request(uuid) IS 'Cria uma nova vaga a partir de uma solicitação aprovada. Valida o tipo de contrato e usa CLT como padrão se inválido. Lida corretamente com approved_by sendo UUID ou nome (text).';
