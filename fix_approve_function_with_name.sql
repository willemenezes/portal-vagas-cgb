-- Função corrigida para aprovar solicitação e criar vaga diretamente
-- Agora salva o nome do aprovador em vez do ID
...
BEGIN
    ...
    -- Se não encontrar nome, usar email ou fallback
    IF approver_name IS NULL THEN
        SELECT email INTO approver_name
        FROM auth.users 
        WHERE id = auth.uid();
